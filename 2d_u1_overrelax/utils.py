...
def topo_from_field(theta):
    ...
    Input shapes:
    - Single sample: [2, lattice_size, lattice_size]
    - Batch: [batch_size, 2, lattice_size, lattice_size]
    
    Output shapes:
    - Single sample: scalar tensor
    - Batch: [batch_size, 1] tensor
    """
    # Check if we have a batch of configurations
    is_batch = theta.dim() == 4
    
    if is_batch:
        batch_size = theta.shape[0]
        result = torch.zeros(batch_size, 1, device=theta.device)
        
        # Process each sample in the batch
        for i in range(batch_size):
            thetaP = plaq_from_field(theta[i])
            thetaP_wrapped = regularize(thetaP)
            result[i, 0] = torch.floor(0.1 + torch.sum(thetaP_wrapped) / (2 * math.pi))
